...
//        Element X_4 = ElementUtils.getGenerator(pairing, generator , typeA1Params, 3, 4).getImmutable();
        ...
        Element R1 = ElementUtils.getGenerator(pairing, generator , typeA1Params, 2, 4).getImmutable();
        Element R2 = ElementUtils.getGenerator(pairing, generator , typeA1Params, 2, 4).getImmutable();


        Element[] R = new Element[n];
        Element[] K = new Element[n];
        for (int i=0;i<n;i++){
            R[i] = ElementUtils.getGenerator(pairing, generator , typeA1Params, 2, 4).getImmutable();
            K[i] = (u[i].mul(h)).powZn(t).mul(R[i]);
        }

        Element K1 = (g.powZn(alf)).mul(g.powZn(a.mul(t))).mul(R1);
        Element K2 = (g.powZn(t)).mul(R2);
